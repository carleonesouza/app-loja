...
exports.createProduct = async (req, res) => {
  ...
  const product = await newProduct.save().then(() => {
    console.log("Product Save Successfully!");
  });
  res
    .status(201)
    .send({ message: "The product has been created successfully !", product });
};
...
exports.updateProduct = async (req, res) => {
  ...
  const changeProduct = await Product.updateOne(
    { _id: req.body._id },
    {
      $set: {
        name: req.body.name,
