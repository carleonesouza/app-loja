...
exports.createVendor = async (req, res) => {
  await new Vendor(req.body)
    .save()
    .then((vendor) => {
      Wallet.findOne({ _id: req.body.walletId }, (err, wallet) => {
        if (err) return err;
...
exports.addAVendorAWallet = async (req, res) => {
  await Wallet.findOne({ _id: req.body.walletId }, (err, wallet) => {
    if (err) return err;
  ...
  await Wallet.findOne({ _id: req.body.walletId }, (err, wallet) => {
    ...
    new Bill(req.body.bill)
      ...
      .then((billSaved) => {
        ...
        return res
          .status(201)
          .json({ message: "The bill has been created successfully !" });
      })
      .catch((err) => {
...
exports.closeAWallet = async (req, res) => {
  ...
  const closeWallet = await Wallet.updateOne(
    { _id: req.body._id },
    {
      $set: {
        status: false,
